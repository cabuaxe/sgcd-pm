...
echo "PM container listening on port 3000 (host nginx proxies port 80 → 3000)"
echo "API: http://217.154.2.230/api/v1/auth/login"
echo "Stakeholder: http://217.154.2.230/stakeholder?token=sgcd-stakeholder-2026"
echo ""
echo "If host nginx is not yet set up, run on VPS:"
echo "  bash /opt/sgcd-pm/infra/setup-host-nginx.sh <domain>"
